...
2. 客户端内嵌 `entryUrls`（一主两备，串行）：腾讯云公网主 → CNB raw 备 → GitHub raw 备。
...
细节见 [docs/design/bootstrap-directory.md](../design/bootstrap-directory.md) 与 [`SPEC.md` §16](../../SPEC.md)。

## 后果

- App 内嵌入口长期稳定；加减机房主要改目录与 publish 双写，不必改客户端常量。
- 发布链必须能更新三处目录镜像，并保证各 serve 上的 index/产物字节一致。
