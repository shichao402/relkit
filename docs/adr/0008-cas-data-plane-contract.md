...
5. **删除 `local`、`http-put` 与 `static-http`。** 内网 ingest 用 `relkit-compatible`。本机演练起真 `relkit-serve`（文档与 `Skeleton()` 对齐 `127.0.0.1:30341`）。`http-put` 的配置形状（`baseUrl`、可选 `uploadUrl`、`tokenEnv`、`timeoutSeconds`）由 `relkit-compatible` 沿用。外部已送达的树把绝对 URL 写进签名文档，不设只读后端类型。
...
- 删 `local` / `static-http` 后，SPEC 的离线兜底是本机 serve；U 盘 / rsync 不再靠一个伪造凭据的后端。外部 URL 只出现在签名文档里。
