...
> 后续决策见 [ADR 0008](../adr/0008-cas-data-plane-contract.md)。下文是当时现场记录，不代表现行后端类型。
...
| `/etc/relkit-agent/relkit-agent.json` | 发布机 | `scripts/deploy/relkit.py install agent` | `init -product` | agent | 启动时 |
