...
下图中 `cas/credentials`、CI 直传与瘦 staged tar 已实现；`Materialize` 和 `artifactTo` / `pointerTo` 尚未实现。整包 staged-put 仍可并行保留。
