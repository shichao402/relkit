...
func HostChecklist() Checklist {
	return Checklist{
		...
		Items: []Item{
			...
			{ID: "gold.cas-planned", Title: "CAS ingest on agent publish", Type: TypeDetect, Detect: &Detect{Kind: "cas-planned"}},
			...
			{ID: "human.vendor-relkit", Title: "vendored relkit clones GitHub and follows main", Type: TypeManual, DependsOn: []string{"cli.present"}, Prompt: "If this repo sparse-checkouts or vendors relkit: clone https://github.com/shichao402/relkit.git and follow main. Pin only with a full commit SHA. Go module path is go.firoyang.com/relkit with replace to ./third_party/relkit. Do not go get it. Ack N/A if there is no vendor tree."},
			{ID: "human.stable-artifact-hash", Title: "release binaries do not inject unique BuildTime", Type: TypeManual, DependsOn: []string{"cli.present"}, Prompt: "Confirm host release ldflags do not inject a unique BuildTime (or equivalent) per build. Same sources should produce the same sha256 so CAS skip-upload and client skip-download can hit."},
			{ID: "human.download-dest-installed", Title: "Download destPath is the installed artifact", Type: TypeManual, DependsOn: []string{"cli.present"}, Prompt: "Confirm SDK Download destPath (or Dart/Node destination) is the already-installed file, not a fresh temp path. Unchanged artifacts skip GET via size+sha256. Ack N/A if this host only downloads a single installer blob and accepts no skip."},
