...
func HostChecklist() Checklist {
	return Checklist{
		...
		Items: []Item{
			...
			{ID: "human.consume-lock-v2", Title: "host pins immutable relkit release artifacts", Type: TypeManual, DependsOn: []string{"cli.present"}, Prompt: "If this repo consumes relkit: keep a relkit.consume/2 lock with release, commit, URL, and SHA-256 for every required SDK/CLI/updater artifact. Install only through the byte-identical scripts/host/relkit_consume.py; source checkout and host-side Go builds are forbidden. Ack N/A if this host does not embed relkit."},
