...
func HostChecklist() Checklist {
	return Checklist{
		...
		Items: []Item{
			...
			{ID: "human.vendor-relkit", Title: "vendored relkit clones GitHub and follows main", Type: TypeManual, DependsOn: []string{"cli.present"}, Prompt: "If this repo sparse-checkouts or vendors relkit: clone https://github.com/shichao402/relkit.git and follow main. Pin only with a full commit SHA. Go module path is firoyang.com/relkit with replace to ./third_party/relkit. Do not go get it. Ack N/A if there is no vendor tree."},
