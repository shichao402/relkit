...
`get` 用于发布工具自身读回现状（校验现有 index、比对 manifest 哈希）。它**不能**由「按文档里的 URL 去下载」替代：`static-http` 后端可描述由外部系统托管的位置；未配写入交接目录时只能做只读校验。判断「客户端真的下载得到吗」是另一件事，属于深度校验的范畴。

### 13.1 三种数据面后端（规范性）

后端类型按发布工具连接的数据面 API 命名。现行实现只定义三种：
...
| `static-http` | 匿名 HTTP GET；对象已由其它系统放置，或只用于镜像校验 | 禁止 |

`local` 与 `http-put` 不属于现行后端。离线或本机演练**必须**启动真实 `relkit-serve` 数据面，或用 `static-http` 声明一个外部已送达的树；禁止用不可访问的 `baseUrl` 伪造发布成功。
...
- `put_pointer` 成功返回后，内容必须对匿名访问者可见（或明确说明最终一致延迟）。
- 文档中的下载 URL 必须匿名可读；需要额外鉴权的 URL 不得写入 RUP 文档。
- `static-http` 若依赖外部放置流程，发布方必须等 URL 实际可读后才能提交 index。
- 自托管 `cas/` 只有在未被 index 引用、没有有效上传租约且超过 `gc.casGrace` 后才可删除；默认保护期 24 小时。
...
设计说明（非规范性）：客户端内嵌的 `entryUrls` 一主多备（例如广州 COS 主 + 异地域第二 COS backend）只托管这份小文档的镜像。同桶多域名不是多个 backend。全网不可达时宿主展示编译期内嵌的 `recovery` 文案与官方手动入口（不经网络）。备援准入见 [ADR 0007](docs/adr/0007-entry-mirror-must-be-reachable-and-cacheable.md)。细节见 [`docs/design/bootstrap-directory.md`](docs/design/bootstrap-directory.md)、[`docs/design/update-ingress-cos.md`](docs/design/update-ingress-cos.md) 与 ADR 0005。
