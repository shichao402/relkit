...
已实现：`init` `keygen` `stage` `inspect` `simulate` `publish` `verify` `agent-guide` `backends`；后端只有 `s3-compatible`（长期钥 query 预签名）、`relkit-compatible`（serve 能力 URL / COPY / HEAD / DELETE）与 `static-http`（只读镜像 / 外部 URL）。CAS 客户端只执行绝对 URL `requests[]`，不签名。

**未实现：`github-release` / `cnb-release` 后端，以及 `yank` / `unyank` / `min-supported`。** 用 `relkit backends` 确认当前构建实到哪一步。推荐对外入口拓扑（自有域名 + COS、迁移 runbook）见仓库 `docs/design/update-ingress-cos.md`；操作侧以 AGENT-GUIDE 为准。**禁止**恢复 `local` / `http-put`、`casCredentials=sts` 或客户端 `sign`。本机演练也使用真实 `relkit-serve` + `relkit-compatible`。
