...
- 给服务端加 HTTP 管理接口（签发/列举/吊销 token）→ 明确不做，理由见手册 §2.3.1。用户要求时先说明「上传 token 不是管理口令」，再问是不是 SSH 不通。操作面板登录（`/-/admin`）也不是这条口：它不能增删产品 token；重置面板走目标机 `init -reset-admin`，不走本 skill。
